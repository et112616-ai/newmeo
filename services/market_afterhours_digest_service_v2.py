...
MARKET_AFTERHOURS_VERSION = (
    "2026-07-28-v2-DAILY-TSE-OTC-CONTRIBUTION"
)
...
def _latest_daily_contribution_rows() -> dict[str, Any]:
    """盤後總覽只讀每日盤後表，不再誤讀盤中 market_contribution_1m。"""
    try:
        from services.supabase_service import (
            get_latest_market_index_contribution_daily,
        )

        return {
            "tse": get_latest_market_index_contribution_daily("tse"),
            "otc": get_latest_market_index_contribution_daily("otc"),
        }
    except Exception as exc:
        _debug("latest daily contribution failed", "| error =", repr(exc))
    return {"tse": {}, "otc": {}}
...
def _normalize_contribution(
    ...
    if not isinstance(contributors, dict):
        contributors = {}
    positive = contributors.get("positive") or []
    negative = contributors.get("negative") or []
    if not isinstance(positive, list):
        positive = []
    if not isinstance(negative, list):
        negative = []
    return {
        "available": bool(row.get("trade_date")),
        ...
        "index_close": round(_safe_float(row.get("index_close")), 4),
        "index_reference": round(
            _safe_float(row.get("index_reference")),
            4,
        ),
        "index_change_points": round(
            _safe_float(row.get("index_change_points")),
            4,
        ),
        "index_change_pct": round(
            _safe_float(row.get("index_change_pct")),
            6,
        ),
        "positive_contribution_points": round(
            _safe_float(row.get("positive_contribution_points")),
            4,
        ),
        "negative_contribution_points": round(
            _safe_float(row.get("negative_contribution_points")),
            4,
        ),
        "net_contribution_points": round(
            _safe_float(row.get("net_contribution_points")),
            4,
        ),
        "positive_count": _safe_int(row.get("positive_count")),
        "negative_count": _safe_int(row.get("negative_count")),
        "flat_count": _safe_int(row.get("flat_count")),
        "universe_rows": _safe_int(row.get("universe_rows")),
        "positive": [
            item for item in positive if isinstance(item, dict)
        ],
        "negative": [
            item for item in negative if isinstance(item, dict)
        ],
        "largest_positive": (
            positive[0]
            if positive and isinstance(positive[0], dict)
            else {}
        ),
        "largest_negative": (
            negative[0]
            if negative and isinstance(negative[0], dict)
            else {}
        ),
        "method": str(row.get("method") or ""),
        "is_official": bool(
            row.get("is_official_component_contribution")
        ),
...
def build_market_afterhours_digest() -> dict[str, Any]:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        daily_contribution_future = executor.submit(
            _safe_call,
            "daily_contribution",
            _latest_daily_contribution_rows,
        )
        ...
        daily_contribution = daily_contribution_future.result()
    ...
    tse_contribution = _normalize_contribution(
        daily_contribution.get("tse") or {},
        "tse",
    )
    otc_contribution = _normalize_contribution(
        daily_contribution.get("otc") or {},
        "otc",
    )

    trade_date = str(tse_contribution.get("date") or "")
    if not trade_date:
    ...
    data_status = [
        ...
        {
            "label": "上市貢獻",
            "available": bool(tse_contribution.get("available")),
            "date": str(tse_contribution.get("date") or ""),
            "source": str(tse_contribution.get("source") or ""),
            "message": "",
        },
        {
            "label": "上櫃貢獻",
            "available": bool(otc_contribution.get("available")),
            "date": str(otc_contribution.get("date") or ""),
            "source": str(otc_contribution.get("source") or ""),
            "message": "",
        },
    ...
    result = {
        ...
        "contribution": {
            "tse": tse_contribution,
            "otc": otc_contribution,
        },
        # 保留 weight 鍵給尚未同步更新的舊圖卡；內容改指向每日上市貢獻，
        # 不再回傳盤中的 market_contribution_1m 快照。
        "weight": {
            "available": bool(tse_contribution.get("available")),
            "date": str(tse_contribution.get("date") or ""),
            "top20_contribution_points": round(
                _safe_float(
                    tse_contribution.get("net_contribution_points")
                ),
                2,
            ),
            "largest": dict(
    ...
    _debug(
        ...
        "| tse_contribution_date =",
        result["contribution"]["tse"]["date"],
        "| otc_contribution_date =",
        result["contribution"]["otc"]["date"],
        "| sec =", result["seconds"],
    )
    return result
