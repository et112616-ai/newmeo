...
STOCK_GROUP_COMPARISON_VERSION = (
    "2026-07-29-v5.1-GROUP-TREND-BEFORE-PRIMARY70"
)
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        print(
            "DEBUG stock_group_comparison_v5_1 | load failed",
            "| path =", str(path),
            "| error =", repr(exc),
            flush=True,
        )
...
def _build_group_trends(
    ...
    for group_name in primary_concepts:
        members = PRIMARY_GROUPS.get(group_name, {})
        rows: list[dict[str, Any]] = []
        for member_id, member_name in members.items():
            frame = frames.get(member_id, pd.DataFrame())
            ...
            previous_close = (
                _safe_float(close_series.iloc[-2])
            ...
            if "Volume" in frame.columns:
                ...
                if len(volume_series):
                    try:
                        volume = _safe_float(
                            volume_series.loc[latest_index]
                        )
                    except Exception:
                        volume = _safe_float(volume_series.iloc[-1])
            rows.append(
                {
                    "stock_id": member_id,
                    "stock_name": member_name,
                    "price": round(latest_close, 4),
                    "previous_close": round(previous_close, 4),
                    "change": round(change, 4),
                    "change_pct": round(change_pct, 4),
                    "volume_shares": round(volume),
                    "volume_lots": round(volume / 1000.0, 2),
                    "data_date": pd.Timestamp(latest_index).strftime(
                        "%Y-%m-%d"
                    ),
                    "is_target": member_id == target_id,
                }
            )
        rows.sort(
            key=lambda row: (
            ...
            trends.append(
                {
                    ...
                    "price_definition": "最新日K收盤價",
                    "volume_definition": "最新日K總量",
                    "data_date": max(
...
def _publish_ratio_chart(comparison: dict[str, Any]) -> str:
    ...
    try:
        ...
        print(
            "DEBUG stock_group_comparison_v5_1 | chart failed",
            "| error =", repr(exc),
            flush=True,
        )
        return ""
...
def build_stock_concept_peer_comparison(
    ...
    trend_member_ids = [
        ...
        for member_id in PRIMARY_GROUPS.get(concept, {})
    ]
    all_ids = list(
        dict.fromkeys(
            [normalized_id, *trend_member_ids, *candidate_ids]
        )
    )
    ...
    group_trends = _build_group_trends(
        primary_concepts=primary_concepts,
        frames=frames,
    ...
    result = {
        ...
        "trend_available": bool(group_trends),
