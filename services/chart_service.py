...
def generate_chip_chart(stock_id: str, stock_name: str, chip_rows: dict[str, list[dict]]) -> str:
    ...
    fig.suptitle(f"{stock_id} {stock_name} 三大法人 10日籌碼", fontsize=14, fontweight="bold", y=0.98)
    ...
    for ax, (title, rows) in zip(axes, sections):
        ...
        ax.text(0.02, 1.08, f"{date} │ {title}當日買賣超：{today:,.0f} 張", transform=ax.transAxes, fontsize=10, fontweight="bold")
